...
module.exports.handler = async (event, context, callback) => {
  try {
    ...
    const update = await preproccessBody(event, callback, validationSchema);

    const updatedReview = await JobReviewManager.updateReview(id, update);

    callback(null, buildResponse(200, updatedReview));
  } catch (error) {
