...
def lambda_handler(event, context):
    try:
        tablesData = ddb.list_tables()

        for ddbTable in tablesData['TableNames']:
            # create backup
            ...
            responseLatest = ddb.list_backups(TableName=ddbTable, TimeRangeLowerBound=datetime(
                lowerDate.year, lowerDate.month, lowerDate.day), TimeRangeUpperBound=datetime(upperDate.year, upperDate.month, upperDate.day))
            ...
            response = ddb.list_backups(TableName=ddbTable, TimeRangeLowerBound=datetime(
                2017, 11, 29), TimeRangeUpperBound=datetime(deleteupperDate.year, deleteupperDate.month, deleteupperDate.day))
            ...
            if latestBackupCount >= 2:
                ...
                while (lastEvalBackupArn != ''):
                    ...
                    response = ddb.list_backups(TableName=ddbTable, TimeRangeLowerBound=datetime(2017, 11, 23), TimeRangeUpperBound=datetime(
                        deleteupperDate.year, deleteupperDate.month, deleteupperDate.day), ExclusiveStartBackupArn=lastEvalBackupArn)
                    if 'LastEvaluatedBackupArn' in response:
                        lastEvalBackupArn = response['LastEvaluatedBackupArn']
                    else:
                        lastEvalBackupArn = ''
